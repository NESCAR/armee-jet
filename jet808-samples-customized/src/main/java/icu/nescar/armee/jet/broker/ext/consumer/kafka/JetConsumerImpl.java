...
public class JetConsumerImpl extends KafkaConsumerImpl<ConsumerRecord<MsgKey, byte[]>> {
    ...
    public void run(){
        ...
        timeout=Long.parseLong(System.getProperty(VmOptions.TIME_OUT));
        for (ConsumerRecord<MsgKey, byte[]> record:records){
            ...
            if(record.key().getMsgId()==0x8114){//msgid是上锁消息下发
                RespLockInfoSettings lockInfo = (RespLockInfoSettings)SerializationUtil.deserialize(record.value());//设置具体的下发信息内容
